...
{
	for (unsigned int i = 0; i < HEXES_NUMBER_ALIGN; ++i) {
		king_attacks[i].lo = 0;
		king_attacks[i].hi = 0;
	}
		...
		for (int rank = RANK_1; rank <= RANK_11; ++rank) {
			...
			for (int row = rank-2; row <= rank+1; ++row) {
				if (Hexbitboard::hex_is_ok(file-1, left+row)) {
					attack.set(file-1 + HEX_A1 + (left+row)*Hexbitboard::RANK_WIDTH);
				}
			}
			for (int row = rank-2; row <= rank+1; ++row) {
			...
			unsigned int pointer = file + HEX_A1 + rank*Hexbitboard::RANK_WIDTH;
